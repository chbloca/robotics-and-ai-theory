...
## cross-correlation (NCC) instead of SSD. You should also observe the number of
## correct matches compared to SSD above.
##
...
## We compute the normalised cross-correlation (NCC) of pixels' intensities
## for all pairs of patches extracted from the two images    
...
for i1 in range(npts1):
    for i2 in range(npts2):
        ...
        a = np.sum((patches1[:,:,i1] - np.mean(patches1[:,:,i1]))*(patches2[:,:,i2] - np.mean(patches2[:,:,i2])))
        b = np.sum((patches1[:,:,i1] - np.mean(patches1[:,:,i1]))**2)
        c = np.sum((patches2[:,:,i2] - np.mean(patches2[:,:,i2]))**2)
        distmat[i1,i2]= a/np.sqrt(b*c)
