...
def RNN(train_data, val_data, train_labels, val_labels, X_test):
    ...
    model = Sequential()
    model.add(LSTM(100, return_sequences = True, input_shape = train_data.shape[1:], kernel_regularizer=regularizers.l1(0.0001), activity_regularizer=regularizers.l1(0.0001)))
    model.add(Dropout(0.5))
    model.add(BatchNormalization())
    
    model.add(LSTM(350, kernel_regularizer=regularizers.l1(0.0001), activity_regularizer=regularizers.l1(0.0001)))
    model.add(Dropout(0.5))
    model.add(BatchNormalization())

    model.add(Dense(train_labels.shape[1], activation = 'softmax', kernel_regularizer=regularizers.l1(0.0001), activity_regularizer=regularizers.l1(0.0001)))  
    
    model.compile(optimizer = 'adam', loss = 'categorical_crossentropy', metrics=['accuracy'])
    model.summary()

    model.fit(train_data, train_labels, batch_size = 32, epochs = 35, validation_data = ([val_data, val_labels]))
